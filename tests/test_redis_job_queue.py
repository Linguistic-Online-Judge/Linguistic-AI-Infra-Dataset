...
def test_redis_queue_rejects_wrong_contract_route(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match="must not configure decode_responses"):
        RedisJobQueue(
            redis_url="redis://localhost:6379/0?decode_responses=true",
            routing_key=_routing_key("contract-a"),
        )
    with pytest.raises(ValueError, match="unsupported Redis connection query override"):
        RedisJobQueue(
    ...
    with pytest.raises(ValueError, match="must not repeat query parameters"):
        RedisJobQueue(
