...
def test_qwen_api_composes_repository_registry_with_one_queue_per_contract(
    tmp_path: Path,
    monkeypatch,
) -> None:
...
def test_qwen_api_composes_explicit_registry_with_one_queue_per_contract(tmp_path, monkeypatch):
    monkeypatch.setattr(qwen_api_module, "RedisJobQueue", _Queue)
    registry_path, challenge_ids = _write_qwen_registry(tmp_path, count=2)
    runtime = qwen_api_module.build_qwen_api(root=tmp_path,
        challenge_registry_path=registry_path, database_path=tmp_path / 'submissions.db',
        redis_url='redis://127.0.0.1:6379/0',
        authenticate=lambda request: Principal('subject-alice'),
        allow_draft_submissions=True, environment='development')
    runtime.store.register_user(auth_subject='subject-alice', public_handle='alice')
    with TestClient(runtime.app) as client:
        assert client.get('/health/ready').status_code == 200
        responses = [
...
def test_qwen_api_rejects_contract_worker_cannot_execute(
    ...
    with pytest.raises((QwenRuntimeAttestationError, ValueError)):
        qwen_api_module.build_qwen_api(
            root=tmp_path,
...
def test_qwen_api_reads_production_postgres_credential_file(tmp_path: Path, monkeypatch) -> None:
    import os
