...
5. 生产执行器需要统一串行调度、模型身份/数据绑定核验、实例互斥，以及跨进程
   的未确认推理保护。当前模板只管理应用进程，没有生成多份自动重启的模型执行器。
   单任务`qwen_worker`不能直接扩展为70个并发消费者。
...
阻止同一提供器再次发送请求。这项修复不等于完成跨进程恢复：强制退出后的持久
保护、旧推理终止确认、故障演练仍属于生产执行器交付工作。
