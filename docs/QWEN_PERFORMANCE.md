...
最新获准的1／32并发固定工作量及内核追踪已完成107次请求并恢复服务，见
[批处理与内核实测](BATCH_KERNEL_RESULTS.md)。本组36次请求吞吐提高13.52倍，生成轨迹
主要时间在矩阵运算；32并发仍有评分重复性差异，不能作为课堂验收通过。

随后完成26次原协议、无重启的分阶段测量，见
[首词元与生成阶段实测](GENERATION_STAGE_RESULTS.md)。本轮德语98.3%的引擎推理时间
处于后续生成阶段。指标来自服务端逐请求计数差，不是浏览器首字时间或课堂负载验收。
